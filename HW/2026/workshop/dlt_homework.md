...
```sql
SELECT MIN(trip_pickup_date_time) AS start_date, MAX(trip_pickup_date_time) AS end_date FROM nyc_taxi_trips
```
...
```sql
SELECT (COUNT(*) FILTER(WHERE payment_type = 'Credit') * 1.0) / COUNT(*) AS credit_card_proportion FROM nyc_taxi_trips
```
...
```sql
SELECT SUM(tip_amount) AS total_tips FROM nyc_taxi_trips
```
